// Bitcoin utilities for the blaketest wallet: BLAKE2b chain, testnet4.
// Derived from testcoin.org. Spends sign with the unified opt-in sighash
// (0x21) by default, which makes them invalid on the SHA256d chain.
...
import { unifiedSighash, SIGHASH_ALL, SIGHASH_UNIFIED, SCRIPT_TYPE_TAPROOT } from './unified.js';
...
function readU32(b) { return (b[0] | (b[1] << 8) | (b[2] << 16) | (b[3] << 24)) >>> 0; }
function readI64(b) { let v = 0n; for (let i = 7; i >= 0; i--) v = (v << 8n) | BigInt(b[i]); return BigInt.asIntN(64, v); }
...
export async function createTransaction(privateKey, utxos, toAddress, amount, feeRate, { unified = true } = {}) {
  ...
  for (const utxo of sortedUtxos) {
    ...
    // (a 65-byte opted-in signature adds a quarter vbyte per input)
  ...
  const vbytes = Math.ceil(10.5 + selectedUtxos.length * (unified ? 57.75 : 57.5) + 2 * 43);
  ...
  // Sign each input with Schnorr. By default the signature opts in to the
  // unified sighash (hash type 0x21): the message commits to every spent
  // amount and script, and a node without the fork cannot reconstruct it.
  // With unified=false this is a plain BIP341 SIGHASH_DEFAULT key-path spend,
  // valid on both chains and therefore replayable.
  const witnesses = [];
  ...
  if (unified) {
    const txForSighash = {
      version: 2,
      locktime: 0,
      inputs: inputs.map(inp => ({ txid: inp.txid, vout: readU32(inp.vout), sequence: readU32(inp.sequence) })),
      outputs: outputsData.map(out => ({ value: readI64(out.value), script: out.script })),
    };
    const spentOutputs = inputs.map((inp, i) => ({ value: inp.value, script: prevoutScripts[i] }));
    const hashType = SIGHASH_ALL | SIGHASH_UNIFIED;
    for (let i = 0; i < inputs.length; i++) {
      const sighash = unifiedSighash(txForSighash, i, hashType, SCRIPT_TYPE_TAPROOT, spentOutputs);
      const signature = schnorr.sign(sighash, privateKey);
      witnesses.push([new Uint8Array([...signature, hashType])]);
    }
  } else {
